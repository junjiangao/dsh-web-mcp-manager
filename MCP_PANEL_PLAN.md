...
本方案已确认。当前阶段仅保存计划文档，将项目纳入 Git 管理，并在 GitHub 创建同名公开仓库、提交和推送文档；不实施插件、不安装插件依赖、不修改宿主或用户 profile。

以下功能、接口、实现步骤和测试均属于后续开发计划，尚未实施或执行。插件开发需另行明确授权。
...
未来实现使用基线版本的类型和公共导出；验收至少覆盖 `0.1.2-rc.1` 与 `0.1.3-alpha.1`。依赖范围显式处理预发布版本，不据此承诺兼容所有未来版本。上述结论是源码兼容性核查，尚非运行测试结果。
...
5. 完成相关测试、两个目标版本的集成验证、打包及中文安装说明。

开发验证使用独立临时 `DSH_HOME`。不直接修改用户现有 Web profile。

预期后续交付包括插件源码、构建与测试脚本、可安装的 `.tgz` 包和中文使用说明。安装示例仅供后续实现验收，不是当前已经可执行的交付物：

```bash
dsh plugin --profile web add ./dsh-web-mcp-manager-0.1.0.tgz
dsh web
```
...
当前阶段仅验收计划文档的 Markdown 结构、需求完整性、版本与接口依据，以及 Git 仓库创建、公开可见性和推送结果；不将这些文档和仓库检查表述为插件功能验证通过。
