...
function parseSecretMap(value: unknown, field: string): Record<string, SecretInput> {
  ...
  for (const [key, entry] of Object.entries(record)) {
    ...
    if (patch.clear !== undefined && typeof patch.clear !== 'boolean') throw new TypeError(`${field}.${key}.clear must be a boolean`)
    if (patch.value !== undefined && typeof patch.value !== 'string') throw new TypeError(`${field}.${key}.value must be a string`)
    if (patch.clear !== true && patch.value === undefined) throw new TypeError(`${field}.${key} must set value or clear it`)
    setOwn(result, key, {
...
function parseReconnect(value: unknown): Partial<ReconnectPolicy> {
  const record = asRecord(value, 'reconnect must be an object')
  const result: Partial<ReconnectPolicy> = {}
  if (record.enabled !== undefined) result.enabled = asBoolean(record.enabled, 'reconnect.enabled')
  for (const key of ['initialDelayMs', 'maxDelayMs', 'maxAttempts'] as const) {
    if (record[key] !== undefined) {
      if (!Number.isSafeInteger(record[key]) || (record[key] as number) < 1) throw new TypeError(`reconnect.${key} must be a positive integer`)
      result[key] = record[key] as number
    }
  }
  if (result.initialDelayMs !== undefined && result.maxDelayMs !== undefined) validateReconnect({ ...DEFAULT_RECONNECT, ...result })
  return result
}

function parseServerPatch(value: unknown): ServerPatch {
...
export function mergeSecretMap(current: Record<string, string>, patch: Readonly<Record<string, SecretInput>> | undefined): Record<string, string> {
  ...
  for (const [key, input] of Object.entries(patch)) {
    ...
    if (input.value !== undefined) setOwn(result, key, input.value)
  }
  return result
}
...
export function cloneServer(server: StoredServer): StoredServer {
  return {
    ...server,
    args: [...server.args],
    env: { ...server.env },
