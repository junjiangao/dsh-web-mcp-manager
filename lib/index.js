...
*/
function registerManagerRpcRoute(ctx, channel, dispatch) {
	const route = {
		kind: "prefix",
		path: channel,
...
async function handleRequest(ctx, channel, dispatch, req, res) {
	if (req.method !== "POST") {
...
async function readBody(req) {
	...
	for await (const chunk of req) {
		...
		if (received > 2097152) throw new BodyTooLargeError("request body too large");
		chunks.push(buffer);
	}
	return Buffer.concat(chunks);
}
/** Decode `<channel>/<endpoint...>` and reject anything outside the manager's endpoint grammar. */
function endpointFrom(rawUrl, channel) {
	let pathname;
	try {
	...
	if (endpoint.split("/").some((segment) => !ENDPOINT_SEGMENT_PATTERN.test(segment))) return void 0;
	return endpoint;
}
	...
	*/
	...
	async start() {
		this.settingsWatchDispose = this.ctx.on("settings/document-updated", (ns) => {
			if (String(ns) !== String("web-mcp-manager")) return;
...
//#endregion
export { Config, DEFAULT_RECONNECT, DEFAULT_TOOL_CALL_TIMEOUT_MS, LEGACY_PLUGIN_MODULE_NAME, MANAGER_NAMESPACE, MCP_MANAGER_CHANNEL, MCP_MANAGER_SETTINGS_NAMESPACE, ManagerSettingsSchema, McpManagerController, PLUGIN_IDENTITY, PLUGIN_MODULE_NAME, apply, defaultDocument, defaultServer, inject, isRpcResult, name, transportOf, validateReconnect, validateServerConfig, validateServerId, validateStoredDocument };
