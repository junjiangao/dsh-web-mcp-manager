...
function SecretFields({ label, entries, t, onChange }) {
    return _jsxs("fieldset", { style: groupFieldsetStyle, children: [_jsx("legend", { style: legendStyle, children: label }), entries.map((entry, index) => _jsxs("div", { style: secretRowStyle, children: [_jsx(Field, { label: t('secretKey'), style: secretFieldStyle, children: _jsx("input", { style: fieldInputStyle, value: entry.key, onChange: event => {
                                const next = [...entries];
                                next[index] = { ...entry, key: event.currentTarget.value };
                                onChange(next);
                            } }) }), _jsx(Field, { label: t('secretValue'), style: secretFieldStyle, children: _jsx("input", { style: fieldInputStyle, type: "password", value: entry.value, disabled: entry.clear, placeholder: entry.clear ? t('secretUnset') : undefined, onChange: event => {
                                const next = [...entries];
                                next[index] = { ...entry, value: event.currentTarget.value };
                                onChange(next);
                            } }) }), _jsxs("div", { style: secretActionsStyle, children: [_jsxs("label", { style: secretClearLabelStyle, children: [_jsx("input", { style: checkStyle, type: "checkbox", checked: entry.clear, onChange: event => {
                                            const next = [...entries];
                                            next[index] = { ...entry, clear: event.currentTarget.checked };
                                            onChange(next);
                                        } }), " ", t('secretUnset')] }), _jsx("button", { type: "button", style: buttonDangerStyle, onClick: () => onChange(entries.filter((_, itemIndex) => itemIndex !== index)), children: t('remove') })] })] }, `${entry.key}-${String(index)}`)), _jsx("button", { type: "button", style: buttonSecondaryStyle, onClick: () => onChange([...entries, { key: '', value: '', clear: false }]), children: t('addEntry') })] });
}
