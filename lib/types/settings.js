...
import { validateMcpConfig } from "./host/mcp-config.js";
...
const _reconnectDefaultsCheck = DEFAULT_RECONNECT;
...
function validateSensitiveKeys(serverId, field, keys) {
    ...
    for (const key of keys) {
        ...
        if (seen.has(key))
            throw new Error(`server ${JSON.stringify(serverId)} ${field} contains duplicate key ${JSON.stringify(key)}`);
        seen.add(key);
    }
}
