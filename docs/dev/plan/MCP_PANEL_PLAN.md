...
- 按服务 ID 分别串行处理生命周期操作，不同服务互不阻塞；启动等待与卸载设置超时上限，避免慢启动或挂起的连接拖住其他服务与卸载。
- 页面请求取消不撤销已经提交的配置。
...
缺陷修复与回归见 `docs/dev/plan/MCP_PANEL_FIX_PLAN.md`（草稿 revision 冲突保护、secret 删除/重命名、参数无损编辑、稳定行 ID、HTML pattern v 模式、轮询顺序与消息分离、按服务生命周期队列）。CI 自动执行类型检查、单元/组件测试与 `lib/` 产物一致性检查；`0.1.2-rc.1` 与 `0.1.3-alpha.1` 双版本的运行时冒烟仍需人工在隔离 profile 中执行。
